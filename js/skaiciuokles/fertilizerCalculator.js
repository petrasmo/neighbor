// js/skaiciuokles/fertilizerCalculator.js
...
export function renderFertilizerCalculator(container) {
        ...
        <div class="space-y-6">
            ...
            <div class="bg-tractorSurface border border-tractorBorder rounded-2xl p-6 md:p-8 space-y-6 shadow-xl">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-5">
                    <div class="space-y-1.5">
                        ...
                        <select id="fert-preset-select" class="w-full h-12 bg-tractorBg border border-tractorPrimary/70 focus:border-tractorPrimary rounded-xl px-3 text-xs md:text-sm font-bold outline-none cursor-pointer" style="color: var(--text-main);">
                            ${presetOptionsHtml}
                        </select>
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-xs font-bold uppercase tracking-wider" style="color: var(--text-main);">Reikalingas grynas N (arba P/K) (kg/ha)</label>
                        <input id="fert-target-active-kg" type="number" step="5" value="80" 
                            class="w-full h-12 bg-tractorBg border border-tractorBorder focus:border-tractorPrimary rounded-xl px-4 text-sm font-bold outline-none" style="color: var(--text-main);">
                    </div>

                    <div class="space-y-1.5">
                        <label class="text-xs font-bold uppercase tracking-wider" style="color: var(--text-main);">Tręšiamas lauko plotas (ha)</label>
                ...
                <div class="bg-tractorBg/90 border border-tractorBorder p-5 rounded-2xl space-y-4">
                    <div class="flex justify-between items-center border-b border-tractorBorder/60 pb-2">
                        <span class="text-xs font-bold uppercase tracking-wider" style="color: var(--text-main);">Trąšų sudėtis (%) ir pirkimo kaina:</span>
                    ...
                    <div class="grid grid-cols-2 sm:grid-cols-5 gap-3">
                        <div class="space-y-1">
                            <label class="text-[11px] font-bold text-green-500 uppercase block">Azotas (N %)</label>
                            <input id="fert-n-perc" type="number" step="0.1" value="34.4" class="w-full h-10 bg-tractorSurface border border-tractorBorder focus:border-tractorPrimary rounded-xl px-3 text-xs font-bold outline-none" style="color: var(--text-main);">
                        </div>
                        <div class="space-y-1">
                            <label class="text-[11px] font-bold text-amber-500 uppercase block">Fosforas (P₂O₅ %)</label>
                            <input id="fert-p-perc" type="number" step="0.1" value="0" class="w-full h-10 bg-tractorSurface border border-tractorBorder focus:border-tractorPrimary rounded-xl px-3 text-xs font-bold outline-none" style="color: var(--text-main);">
                        </div>
                        <div class="space-y-1">
                            <label class="text-[11px] font-bold text-blue-500 uppercase block">Kalis (K₂O %)</label>
                            <input id="fert-k-perc" type="number" step="0.1" value="0" class="w-full h-10 bg-tractorSurface border border-tractorBorder focus:border-tractorPrimary rounded-xl px-3 text-xs font-bold outline-none" style="color: var(--text-main);">
                        </div>
                        <div class="space-y-1">
                            <label class="text-[11px] font-bold text-yellow-400 uppercase block">Siera (S %)</label>
                            <input id="fert-s-perc" type="number" step="0.1" value="0" class="w-full h-10 bg-tractorSurface border border-tractorBorder focus:border-tractorPrimary rounded-xl px-3 text-xs font-bold outline-none" style="color: var(--text-main);">
                        </div>
                        <div class="space-y-1 col-span-2 sm:col-span-1">
                            <label class="text-[11px] font-bold text-amber-500 uppercase block">Kaina (€/t su PVM)</label>
                            <input id="fert-price-ton" type="number" step="10" value="340" class="w-full h-10 bg-tractorSurface border border-tractorBorder focus:border-amber-400 rounded-xl px-3 text-xs text-amber-500 font-mono font-bold outline-none">
                        </div>
                    </div>
                </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div class="bg-tractorPrimary/20 border-2 border-tractorPrimary p-5 rounded-2xl space-y-1 shadow-xl">
                    ...
                    <div class="text-3xl font-black font-mono" id="res-physical-rate" style="color: var(--text-main);">232.6 kg/ha</div>
                    <p class="text-xs text-green-500 font-semibold" id="res-liquid-rate-desc">~0.47 didmaišio/ha</p>
                </div>
                ...
                <div class="bg-tractorSurface border border-tractorBorder p-5 rounded-2xl space-y-1">
                    ...
                    <div class="text-2xl font-bold font-mono" id="res-total-fert-weight" style="color: var(--text-main);">9.30 t</div>
                    <p class="text-xs" id="res-bigbags-count" style="color: var(--text-muted);">Reikės 19 didmaišių (po 500kg)</p>
                </div>

                <div class="bg-tractorSurface border border-tractorBorder p-5 rounded-2xl space-y-1">
                    <span class="text-[11px] uppercase font-bold text-slate-400 tracking-wider">Tręšimo išlaidos</span>
                    <div class="text-2xl font-bold text-amber-500 font-mono" id="res-total-fert-cost">3 162.00 €</div>
                    <p class="text-xs" id="res-fert-cost-per-ha" style="color: var(--text-muted);">Savikaina: 79.05 €/ha</p>
                </div>

                <div class="bg-tractorSurface border border-tractorBorder p-5 rounded-2xl space-y-1">
                    <span class="text-[11px] uppercase font-bold text-slate-400 tracking-wider">1 kg gryno N savikaina</span>
                    <div class="text-2xl font-bold text-green-500 font-mono" id="res-cost-per-kg-active">0.99 €/kg N</div>
                    <p class="text-xs text-slate-400">Efektyvumo rodiklis</p>
                </div>
            </div>

            <!-- KITŲ ELEMENTŲ IŠBERIMAS -->
    ...
    document.getElementById('fert-preset-select')?.addEventListener('change', (e) => {
        ...
        if (preset) {
            ...
            if (preset.isLiquid) liquidBadge?.classList.remove('hidden');
            else liquidBadge?.classList.add('hidden');
...
function calculateFertilizerNorm() {
    ...
    const elPhys = document.getElementById('res-physical-rate');
    const elLiq = document.getElementById('res-liquid-rate-desc');
    const elBags = document.getElementById('res-bigbags-count');

    if (isLiquid) {
        const liquidLitersHa = physicalRateKgHa / 1.32;
        if (elPhys) elPhys.textContent = `${physicalRateKgHa.toFixed(1)} kg/ha`;
        if (elLiq) elLiq.textContent = `💧 Atitinka: ${liquidLitersHa.toFixed(1)} l/ha`;
        if (elBags) elBags.textContent = `Viso: ${Math.round(totalTons * 1000 / 1.32).toLocaleString('lt-LT')} litrų`;
    } else {
        if (elPhys) elPhys.textContent = `${physicalRateKgHa.toFixed(1)} kg/ha`;
        if (elLiq) elLiq.textContent = `~${(physicalRateKgHa / 500).toFixed(2)} didmaišio/ha`;
        if (elBags) elBags.textContent = `Reikės ${bigBags500kg} didmaišių (po 500 kg)`;
    }
    ...
    if (pPerc > 0 || kPerc > 0 || sPerc > 0) {
        ...
        if (breakdownGrid) {
            breakdownGrid.innerHTML = `
                <div class="bg-tractorBg p-3 rounded-xl border border-tractorBorder"><span class="text-green-500 font-bold block">Grynas N</span><strong class="text-sm" style="color: var(--text-main);">${((physicalRateKgHa * (nPerc / 100)) * areaHa).toFixed(1)} kg</strong></div>
                <div class="bg-tractorBg p-3 rounded-xl border border-tractorBorder"><span class="text-amber-500 font-bold block">Fosforas P₂O₅</span><strong class="text-sm" style="color: var(--text-main);">${((physicalRateKgHa * (pPerc / 100)) * areaHa).toFixed(1)} kg</strong></div>
                <div class="bg-tractorBg p-3 rounded-xl border border-tractorBorder"><span class="text-blue-500 font-bold block">Kalis K₂O</span><strong class="text-sm" style="color: var(--text-main);">${((physicalRateKgHa * (kPerc / 100)) * areaHa).toFixed(1)} kg</strong></div>
                <div class="bg-tractorBg p-3 rounded-xl border border-tractorBorder"><span class="text-yellow-400 font-bold block">Siera S</span><strong class="text-sm" style="color: var(--text-main);">${((physicalRateKgHa * (sPerc / 100)) * areaHa).toFixed(1)} kg</strong></div>
            `;
        }
    } else {
